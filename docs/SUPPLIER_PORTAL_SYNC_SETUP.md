...
For portal-button syncs, the worker keeps the exact supplier cost before VAT, applies 15% VAT once, and rounds only the final selling price to the nearest R1. Each supplier scraper has an explicit ex-VAT or VAT-inclusive price mapping so an already VAT-inclusive portal value is first converted back to its source cost instead of receiving VAT a second time.
