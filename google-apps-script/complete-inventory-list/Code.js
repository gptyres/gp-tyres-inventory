...
function setupTwoWayInventorySync() {
...
function repairInventorySyncTrigger() {
  ...
  try {
    ...
    // Running from clasp or direct function execution has no UI.
  }
}
...
function syncPortalItemsToSheet_(sheet, items, reason) {
  ...
  try {
    ...
    const rangeRowCount = Math.max(1, lastRow - GP_SYNC_CONFIG.firstDataRow + 1);
    const portalIdValues = sheet.getRange(GP_SYNC_CONFIG.firstDataRow, GP_SYNC_CONFIG.portalIdColumn, rangeRowCount, 1).getValues();
    const visibleValues = sheet.getRange(GP_SYNC_CONFIG.firstDataRow, 1, rangeRowCount, GP_SYNC_CONFIG.readColumnCount).getValues();
    ...
    items.forEach((item) => {
      ...
      const rowFingerprint = makePortalRowFingerprint_(rowValues);
      const rowNumber = existingRowsByPortalId[portalId]
        || existingRowsByFingerprint[rowFingerprint]
      ...
      existingRowsByFingerprint[rowFingerprint] = rowNumber;
...
function replaceSupplierCatalogSheet_(payload) {
  const allowedSheets = {
    ...
    EXCLUSIVE_TYRES_NEW: 'SUPPLIER_EXCLUSIVE_TYRES_NEW',
