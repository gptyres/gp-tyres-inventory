  ...
  return <>
    <section className="mb-4 rounded-xl border border-gp-border bg-gp-panel p-3">
      ...
      <div className="mt-3 flex flex-wrap gap-2">{TECHNICIANS.map((technician) => {
        ...
        const status = activeBreak ? `${breakLabel(activeBreak.break_type)} · ${elapsedTimeLabel(activeBreak.started_at, null, now)}` : busyOnJob ? 'Busy on job' : 'Available';
        const tone = activeBreak ? 'border-amber-500/40 bg-amber-500/10 text-amber-300' : busyOnJob ? 'border-blue-500/40 bg-blue-500/10 text-blue-300' : 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300';
...
const TechnicianHistoryPanel: React.FC<{ jobs: WorkshopJob[]; breaks: WorkshopTechnicianBreak[]; now: number }> = ({ jobs, breaks, now }) => {
  ...
  return <section className="mt-4 rounded-2xl border border-gp-border bg-gp-dark/70 p-3"><div className="mb-3"><h2 className="text-xs font-black uppercase tracking-wider text-white">Technician activity history</h2><p className="mt-0.5 text-[10px] text-gp-text-muted">Search saved job and availability records to see who was busy, when, and for how long.</p></div><div className="mb-3 grid gap-2 sm:grid-cols-2"><select aria-label="Search technician history" value={technician} onChange={(event) => setTechnician(event.target.value)} className="rounded-lg border border-gp-border bg-gp-input px-3 py-2 text-sm font-bold text-white outline-none focus:border-gp-red"><option value="">All technicians</option>{TECHNICIANS.map((name) => <option key={name} value={name}>{name}</option>)}</select><input aria-label="History date" type="date" value={date} onChange={(event) => setDate(event.target.value)} className="rounded-lg border border-gp-border bg-gp-input px-3 py-2 text-sm font-bold text-white outline-none focus:border-gp-red" /></div><div className="grid gap-3 xl:grid-cols-2"><div><p className="mb-2 text-[10px] font-black uppercase tracking-wider text-gp-text-muted">Job time</p><div className="space-y-2">{jobHistory.map((job) => <div key={job.id} className="rounded-lg border border-gp-border bg-gp-panel p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-black text-white">{technicianList(job).join(', ') || 'Unassigned'} · {job.customer_name}</p><p className="mt-0.5 truncate text-[10px] text-gp-text-muted">{job.job_number} · {job.service_type}</p></div><span className="font-mono text-xs font-black text-amber-300">{elapsedTimeLabel(job.started_at, job.completed_at, now)}</span></div><p className="mt-2 text-[10px] text-gp-text-muted">Started {timeLabel(job.started_at)} · {job.completed_at ? `Finished ${timeLabel(job.completed_at)}` : 'Still active'}</p></div>)}{!jobHistory.length && <p className="rounded-lg border border-dashed border-gp-border p-3 text-center text-[10px] font-bold uppercase tracking-wider text-gp-text-muted">No job activity found</p>}</div></div><div><p className="mb-2 text-[10px] font-black uppercase tracking-wider text-gp-text-muted">Break & collection time</p><div className="space-y-2">{breakHistory.map((item) => <div key={item.id} className="rounded-lg border border-gp-border bg-gp-panel p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-white">{item.technician} · {breakLabel(item.break_type)}</p><span className="font-mono text-xs font-black text-amber-300">{elapsedTimeLabel(item.started_at, item.ended_at, now)}</span></div><p className="mt-2 text-[10px] text-gp-text-muted">Started {timeLabel(item.started_at)} · {item.ended_at ? `Returned ${timeLabel(item.ended_at)}` : 'Still unavailable'}</p></div>)}{!breakHistory.length && <p className="rounded-lg border border-dashed border-gp-border p-3 text-center text-[10px] font-bold uppercase tracking-wider text-gp-text-muted">No availability activity found</p>}</div></div></div></section>;
};
...
const JobForm: React.FC<{ currentUser: string; agents: string[]; form: WorkshopJobInput; setForm: React.Dispatch<React.SetStateAction<WorkshopJobInput>>; onClose: () => void; onSubmit: (event: React.FormEvent) => void; busy: boolean }> = ({ currentUser, agents, form, setForm, onClose, onSubmit, busy }) => {
  const set = (field: keyof WorkshopJobInput, value: string | number) => setForm((current) => ({ ...current, [field]: value }));
  return <QuickIntakeForm currentUser={currentUser} agents={agents} form={form} setForm={setForm} onClose={onClose} onSubmit={onSubmit} busy={busy} />;
